...
@Component
public class CGR153_ADT_QMImageDownloader extends AbstractDownloader implements Downloader {

	private static final Logger logger = LoggerFactory.getLogger(CGR153_ADT_QMImageDownloader.class);
	...
	public void download(String target, String uid, Map<String, Object> params) throws Exception {
		...
		logger.info(String.format(">>>> CGR152_ADT_HybridTopperImageDownloader Start downloading from %s : %s", target, uid));
		...
		try {
			...
			orderNo = iso88591ToUtf8(request.getParameter("order_no")); //유니폼 번호가 orderno로 넘어올 예정
			procId = iso88591ToUtf8(request.getParameter("proc_id"));
			fileCa = iso88591ToUtf8(request.getParameter("FILECA"));
		...
		String filePath = CowayFtpFilePath.getqmImageFolder(jobDate);
		String fileName = CowayFtpFileName.getQMImageName(jobDate, orderNo, imgSeq, fileCa);
