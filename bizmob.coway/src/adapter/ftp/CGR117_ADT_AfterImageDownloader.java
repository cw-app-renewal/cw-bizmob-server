...
@Component
public class CGR117_ADT_AfterImageDownloader extends AbstractDownloader implements Downloader {

	private static final Logger logger = LoggerFactory.getLogger(CGR117_ADT_AfterImageDownloader.class);
	...
	public void download(String target, String uid, Map<String, Object> params) throws Exception {
		...
		logger.info(String.format(">>>> CGR117_ADT_AfterImageDownloader Start downloading from %s : %s", target, uid));
		...
		try {
			...
			logger.info(">>>> params fileStartPos = " + fileStartPos + 
					", imgSeq = " + imgSeq + "orderNo = " + orderNo);
		} catch ( Exception e ) {
		...
		String filePath = CowayFtpFilePath.getAfterFolder(orderNo);
		String fileName = CowayFtpFileName.getAfterImgName(orderNo, imgSeq);
