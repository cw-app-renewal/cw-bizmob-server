...
@Component
public class CGR101_ADT_InstallImageDownloader extends AbstractDownloader implements Downloader {

	private static final Logger logger = LoggerFactory.getLogger(CGR101_ADT_InstallImageDownloader.class);
	...
	public void download(String target, String uid, Map<String, Object> params) throws Exception {
		...
		logger.info(String.format("======== CGR101_ADT_InstallImageDownloader Start downloading from %s : %s ", target, uid));
		...
		//install 사진의 경우, 
		//file_name = orderNo (고객주문번호)
		//file_path = 2 (고객설치사진구분) 
		...
		try {
		    ...
		    orderNo = new String(request.getParameter("order_no").getBytes("iso-8859-1"), "utf-8");
		...
		String fileName = CowayFtpFileName.getInstallImgName(orderNo, imgSeq);
