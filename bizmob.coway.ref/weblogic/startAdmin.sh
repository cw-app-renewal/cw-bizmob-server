...
########## Custom Args ##########
#Common start Args
	USER_MEM_ARGS="-D:${DOMAIN_NAME}_${SERVER_NAME}  -Xms512m -Xmx512m -XX:MaxPermSize=128m"
#64bit JDK
	## only HP
...
#HeapDump
	...
	#USER_MEM_ARGS="${USER_MEM_ARGS} -XX:+HeapDump -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${LOG_DIR}/heapdump"
...
nohup ${DOMAIN_HOME}/bin/startWebLogic.sh >> ${LOG_DIR}/${LOG_NAME}.out &

sleep 2
tail -f $LOG_DIR/$LOG_NAME.out
